...
logoData.get().then((querySnapshot) => {
    ...
    displayInfo(logos[0].title + "—" + logos[0].category, logos[0].url);


});

function displayInfo(title, link) {
...
document.getElementById("work-carousel").addEventListener('scroll', function () {
    ...
    displayInfo(logos[targetProject].title + "—" + logos[targetProject].category, logos[targetProject].url);
